...
export default function Todo() {
  ...
  const { isLoading, data, isFetching } = useCustomQuery({
    queryKey: [
      `todo-page-${page}-size-${pageSize}-sortBy-${sortBy}-version-${queryVersion}-sortBy-${sortBy}`,
    ],
  ...
  if (isLoading) return <TodoSkeleton from="TodosPage" />;
  /* -------------------------------- Handlers -------------------------------- */
        ...
        data.data.map(
          (
            ...
            return (
              <div
                key={todo.id}
                className="flex items-center jsutify-between hover:bg-gray-100 duration-300 p-3 rounded-md even:bg-gray-100"
              >
                <p className="w-full font-semibold">
                  {pageIndex + 1} - {todo.attributes.title}
                </p>
              </div>
            );
          }
